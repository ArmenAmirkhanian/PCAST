 ...
 *
 * Proximity override: if the nearest station is within 3 miles of the target,
 * only that station's data is used (weight = 1, all others = 0).
 *
...
import { haversineMiles, computeIdwWeights, PROXIMITY_THRESHOLD_MILES, NEARBY_THRESHOLD_MILES } from '../weather';
...
describe('runModel – IDW-blended Chicago weather normals (July 15)', () => {
  ...
  beforeAll(() => {
    ...
    // IDW power-2 weights (with 3-mile proximity override via shared utility)
    const rawWeights  = withDist.map(s => 1 / (s.distanceMiles ** 2));
    const sumW        = rawWeights.reduce((a, b) => a + b, 0);
    const normWeights = computeIdwWeights(withDist.map(s => s.distanceMiles));
    stations = withDist.map((s, i) => ({
      ...s,
      rawWeight:  rawWeights[i] / sumW,
  ...
  it('produces finite, physically plausible temperatures for all 24 hours', () => {
    ...
    // Station table
    const proximityOverride = stations[0].distanceMiles <= PROXIMITY_THRESHOLD_MILES;
    const nearbyCount = stations.filter(s => s.distanceMiles <= NEARBY_THRESHOLD_MILES).length;
    const nearbyOverride = !proximityOverride && nearbyCount >= 2 && nearbyCount < stations.length;
    const weightingLabel = proximityOverride
      ? `Proximity Override (nearest ≤ ${PROXIMITY_THRESHOLD_MILES} mi → single station)`
      : nearbyOverride
        ? `Nearby Override (${nearbyCount} stations ≤ ${NEARBY_THRESHOLD_MILES} mi → ${nearbyCount}-station IDW)`
        : 'Inverse-Distance² Weighting (all 3 stations)';
    console.log(`\n NEAREST STATIONS – ${weightingLabel}\n`);
