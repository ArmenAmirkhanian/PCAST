...
`.trim(),
  ...
  liveForecastExplanation: `
The live forecast option replaces the 30-year climate normals with NOAA's operational gridded forecast, published by the National Weather Service through the National Digital Forecast Database (NDFD). Rather than a station record, the NDFD is a continuous analysis field: forecaster-edited grids covering the United States at roughly 2.5 km resolution, from which the values at the project coordinates are read directly. The tool retrieves air temperature, wind speed, and sky cover for the 72 hours beginning at the specified construction start time, and converts them to the same units the thermal model consumes ($^{\\circ}\\mathrm{C}$, m/s, and percent cloud cover).

The distinction from climate normals is fundamental and affects how the results should be read. A climate normal is a statistical expectation: the temperature that *typically* occurs at that hour and place, carrying no information about the specific day being analysed. A forecast is a single deterministic realisation: the temperature the atmosphere is currently predicted to produce on that specific day. Normals therefore describe a representative design condition, while the forecast describes an operational one. A slab analysed against normals answers "is this detail generally sound here in August?"; the same slab analysed against the forecast answers "should we place this Thursday morning?"

Because forecast skill decays rapidly at hourly resolution, the option is restricted to a construction start date of today or tomorrow. Beyond roughly three days, the hour-by-hour structure that drives thermal gradient development — the timing of the diurnal peak, the overnight minimum, the passage of cloud cover — is no longer resolved with useful confidence, and the climatological expectation becomes the better estimator. The forecast horizon and the tool's 72-hour analysis window are matched deliberately so that every modelled hour is covered by issued forecast data.

Two consequences follow for interpretation. First, forecast-driven results are perishable: NOAA reissues the grids roughly hourly, so an analysis run against a forecast is valid only for the issuance time recorded with it, and should be re-run if conditions are revised. Second, when the construction start time has already passed at the moment of the lookup, the leading hours are not available from the forecast and are held at the earliest issued value; those hours are flagged in the hourly table and should be treated as an approximation rather than a prediction.
`.trim()
} as const;
...
The joint does not exist until the slab is saw-cut. Before the saw-cut time the panel is continuous and is modelled as an infinite slab: there is no free edge to relieve curling and no joint to redistribute force, so both thermal actions act at their fully-restrained maxima and $K_I=0$. From the saw-cut hour onward the transverse joint is active, the finite-length edge relief and joint compatibility apply, and the joint spacing influences the result. The saw-cut hour is derived from the placement time (Project Info) and the expected saw-cutting time (Slab Layout). This makes the analysis a direct test of whether the early-age thermal tension cracks the slab before the joint is cut.

That infinite-slab idealisation only holds while the section is intact. Concrete cannot carry more tension than it has, so each hour the creep-adjusted demand on the more tensile fibre is checked against the maturity-based tensile strength $f_t(t)$, and at the first hour where

$$\\sigma_{\\max}(t)\\;\\ge\\;f_t(t)$$

while the slab is still continuous, a natural (uncontrolled) transverse crack is declared: the slab breaks. That hour is re-solved, and every later hour is solved, as a finite panel containing a full-depth crack. The crack is a free edge with no ligament left to transfer force, so the axial restraint collapses from the fully-restrained $-E'\\varepsilon_0$ to the friction-limited $E'\\varepsilon_0\\left(1/\\cosh\\beta L-1\\right)$ accumulated over the panel, and $K_I$ is reported as zero because the crack has already run through the section — there is no crack tip inside it for the saw-cut geometry functions to describe. A crack is permanent: cutting the joint later cannot restore continuity, so the cracked regime governs the remainder of the analysis. The check is only possible where a strength is available; hours without one are skipped, and a run with no strength history at all is reported as such rather than silently carrying an unbounded infinite-slab stress.

Because the crack forms where the section is weakest — taken to be the location the joint was to be cut — the post-crack panel length is the specified joint spacing. The model forms a single crack and does not subdivide the panel further, so hours where the relieved slab still reaches its strength are reported as a warning that additional cracking is expected. Comparing the natural-crack hour with the proposed saw-cut hour is what the tool reports as its verdict: a saw-cut that precedes the crack controls the crack location, while one that follows it does not.
